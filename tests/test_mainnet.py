...
# ─────────────────────────────────────────────────────────────────────────────
# Live integration tests (require RPC_URL + PRIVATE_KEY; auto-skipped in CI)
# ─────────────────────────────────────────────────────────────────────────────


@pytest.fixture(scope="module")
def _live_rpc_url() -> str:
...
@pytest.fixture(scope="module")
def _live_wallet_config(_live_rpc_url: str, _live_private_key: str):
    from vault.wallet_config import WalletConfig
    return WalletConfig(private_key=_live_private_key, rpc_url=_live_rpc_url)
...
class TestLiveWallet:
    ...
    def test_wallet_has_address(self, _live_wallet_config):
        addr = _live_wallet_config.account.address
        assert addr.startswith("0x")
        assert len(addr) == 42

    def test_balance_is_non_negative(self, _live_wallet_config):
        balance = _live_wallet_config.get_balance_eth()
        assert isinstance(balance, float)
        assert balance >= 0.0

    def test_network_id(self, _live_rpc_url: str):
        from web3 import Web3
        w3 = Web3(Web3.HTTPProvider(_live_rpc_url))
