...
pub fn build_cfg(disasm: &Disassembly) -> CFG {
    ...
    for (bid, &start_idx) in sorted_leaders.iter().enumerate() {
        let end_idx = sorted_leaders.get(bid + 1).map(|&n| n - 1).unwrap_or(instrs.len() - 1);
    ...
    let start_off_to_bid: FxHashMap<usize, usize> = blocks.iter()
        .map(|b| (b.start_offset, b.id)).collect();
    ...
    for (bid, block) in blocks.iter().enumerate() {
        ...
        if has_fallthrough && bid + 1 < block_count { succs[bid].push(bid + 1); }
        ...
        if is_jump(last_op) && last_idx > 0 {
            ...
            if let Some(target_offset) = prev.imm_u256.map(|v| v as usize) {
                if let Some(&target_bid) = start_off_to_bid.get(&target_offset) {
                    if !succs[bid].contains(&target_bid) { succs[bid].push(target_bid); }
                }
            }
        }
    }
    ...
    for bid in 0..block_count {
        ...
        for &sid in &succs_copy { blocks[sid].predecessors.push(bid); }
    }

    let offset_to_block: Vec<(usize, usize)> = blocks.iter()
        .map(|b| (b.start_offset, b.id)).collect();
