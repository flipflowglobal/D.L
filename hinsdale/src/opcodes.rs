// src/opcodes.rs — Complete EVM Opcode Table (Frontier → Cancun)
use std::fmt;
...
pub enum OpcodeCategory {
    Stop, Arithmetic, Comparison, Bitwise, Sha3,
    EnvInfo, BlockInfo, Stack, Memory, Storage,
    Flow, Log, System, Invalid,
}
...
macro_rules! op {
    ($code:expr,$mn:expr,$sin:expr,$sout:expr,$imm:expr,$cat:expr,$desc:expr) => {
        Opcode { code: $code, mnemonic: $mn, stack_in: $sin, stack_out: $sout,
                 imm_bytes: $imm, category: $cat, description: $desc }
    };
}
use OpcodeCategory::*;
...
pub fn lookup(byte: u8) -> Opcode {
    match byte {
        ...
        0x04 => op!(0x04,"DIV",         2, 1,  0, Arithmetic, "a / b"),
...
pub fn is_push(byte: u8) -> bool { (0x60..=0x7f).contains(&byte) }
pub fn push_size(byte: u8) -> u8 { if is_push(byte) { byte - 0x5f } else { 0 } }
